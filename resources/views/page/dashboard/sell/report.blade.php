<x-layout.dashboard class="dashboard-index" title="Report">
    ...
    {{-- Navbar --}}
    <nav>
        <a href="{{ route('dashboard.buy.report') }}">Report Buy</a>
        @if (Auth::user()->role->id == 1)
            <a href="{{ route('dashboard.sell.report') }}" class="active">Report Sell</a>
        @endif
    </nav>
    
    {{-- Filter --}}
    <x-form style="font-size: .85em" action="{{ route('dashboard.sell.report') }}" method="GET">
        <div class="buttons">
            <x-input type="date" name="from" value="{{ request('from') }}" />
            <x-input type="date" name="to" value="{{ request('to') }}" />
            <a href="{{ route('dashboard.sell.report') }}" class="button transparent">Clear</a>
    ...
    @if (request('from') && request('to'))
        <a style="display: flex; gap: .4em; align-self: flex-end" href="{{ route('dashboard.sell.report.download', ['from' => request('from'), 'to' => request('to')]) }}" class="button transparent"><i class="fa-solid fa-print"></i>Download</a>
    @else 
        <a style="display: flex; gap: .4em; align-self: flex-end" href="{{ route('dashboard.sell.report.download') }}" class="button transparent"><i class="fa-solid fa-print"></i>Download</a>
    @endif
